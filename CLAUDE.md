...
- ⚠️ **La ricevuta di un giorno il portale Alloggiati Web la genera solo il GIORNO DOPO** (detto dall'utente: sul portale quella disponibile è l'"ultima ricevuta", cioè quella dell'invio precedente). Appena fatto un invio, quella di oggi NON esiste ancora.
- **Dopo "Invia tutto" la ricevuta si salva DA SOLA** (settembre 2026, richiesta dell'utente: un solo pulsante): `invia()` → `salvaRicevutaDopoInvio(struttura)` → `/api/ricevuta` con `dopoInvio:true`, che cerca l'ultima ricevuta **a partire da IERI** (fino a 7 giorni, poi il giorno dell'ultimo invio in Archivio **escluso oggi**) = quella dell'invio precedente. Quella di oggi si salverà al prossimo invio. Se quell'ultima ricevuta è già in `ricevute_fatte` (su Drive, o in qualunque modo se Drive non è attivo) risponde `giaFatta` e non la risalva (niente doppioni su Drive); se non esiste (primo invio) risponde `nessuna` — nessuno dei due è un errore. Un secondo tentativo dopo 5 s solo se qualcosa va storto; poi ⚠️ con "🧾 Riprova a salvare la ricevuta". Nello Storico gli invii di oggi mostrano "🧾 ricevuta pronta da domani" invece di "da fare". Il pulsante manuale nelle Impostazioni resta come il portale (senza data parte da oggi).
...
- **Check-in di gruppo** (settembre 2026, dal caso di un'ospite con 6 persone): la lettura "più documenti insieme" (una foto per volta, `kind:'auto'`) vale per OGNI ospite, non solo per il primo — prima, dal secondo in poi, si usava `kind:'documento'` che riconosce UNA persona sola e gli altri sparivano. Se non si riconosce niente resta sempre una via d'uscita (riquadro d'avviso + pulsante "Scrivi i dati a mano") e, se tutte le letture sono andate in errore, il messaggio è quello vero (foto pesanti / connessione) invece del generico "nessun documento riconosciuto". Le foto che il browser non sa aprire (HEIC di iPhone viste da Android/PC) vengono segnalate SUBITO al caricamento. `beforeunload` avvisa prima di perdere tutto ricaricando la pagina.
- **"Aggiungi altri ospiti" dopo l'invio**: sulla schermata finale del check-in c'è il pulsante; riapre il modulo tenendo le date, salta titolare/contratto/selfie (già fatti dal titolare) e manda una seconda scheda con `aggiunta:true` — l'host la vede con la pastiglia "➕ ospiti aggiunti dopo il primo check-in". Il check-in si può ora fare durante tutto il soggiorno (arrivo fino a un mese fa, purché la partenza non sia passata): prima era ammesso solo il giorno d'arrivo o quello dopo.
...
- **Calendario dal telefono** (settembre 2026): toccando un giorno il dettaglio si porta a vista da solo (`calMostraDettaglio`, lampeggia un attimo) — prima restava sotto la piega e sembrava che il tocco non facesse niente; sul telefono ogni casella mostra 3 barrette invece di 2. **I nomi si toccano**: già registrato → la sua voce nello Storico (`calApriRegistrato`, che usa `id="storico-<ts>"` e la classe `.evidenzia`), ancora in attesa → la scheda del check-in (`calApriInAttesa` → `apriDettaglioCheckin`), con il ➕ Registra anche dentro la riga. Stesse regole per le righe dell'agenda "prossimi giorni" e della scheda "Da fare".
- **Home "Da fare"**: solo titolare + 👤 conteggio ospiti, pulsante a 3 stati (Registra/Da inviare/Riaggiungi) — ora è `pendingAzione()`, condiviso con il calendario; il tocco sulla riga apre la scheda del check-in.
- **Spazio in fondo alle pagine**: `.wrap` ha un padding-bottom per la tabbar flottante (prima copriva l'ultima scheda) e `addBooking()` scende in fondo alla pagina SOLO se si è nella scheda Registra (all'avvio si deve restare in cima alla Home).
