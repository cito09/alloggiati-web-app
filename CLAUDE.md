...
- **La ricevuta si salva DA SOLA dopo "Invia tutto"** (settembre 2026, richiesta dell'utente: un solo pulsante, non deve ricordarsi il secondo tocco): `invia()` → `salvaRicevutaDopoInvio(struttura, giorno)` con struttura e giorno (ora italiana, `oggiItalia()`) fissati al momento dell'invio. Si chiede SEMPRE la data esplicita dell'invio (mai la ricerca all'indietro, che darebbe la ricevuta di un altro giorno se il portale non ha ancora registrato l'invio). Tentativi dopo 4 s, 8 s e 15 s; se falliscono tutti, nel pannello resta ⚠️ con "🧾 Riprova a salvare la ricevuta". Lo script Apps Script crea sempre un file NUOVO: con due invii nello stesso giorno su Drive ci sono due `ricevuta_<giorno>_…pdf`, il più recente è quello completo.
